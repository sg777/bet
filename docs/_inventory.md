...
| `cli-print.md` (was `print.md`) | REWRITTEN   | Rewritten May 11 against `print.c` + `bet.c` + `poker_vdxf.c`. Now covers all five commands (`print_id`, `print_table_key`, `print`, `print_keys`, `show`), all five `print_id` types (`table`/`dealer`/`player`/`dealers`/`cashiers`) with their aliases, the restricted key set for `./bet print`, and the `T_B_P*_DECK_KEY` / `T_CARD_BV_KEY` migration to the cashier id. | `docs/reference/cli-print.md` (Phase 2 move)   |
